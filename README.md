...
회원정보 수정 기능에서는
사용자가 변경한 정보만 전달받아 DB에 반영하고,
수정 완료 후에는 다시 회원 정보를 조회하여 세션 정보를 갱신함으로써
변경 사항이 즉시 반영되도록 처리했습니다.

회원 탈퇴 기능은
일반 로그인 사용자와 소셜 로그인 사용자를 구분하여 처리했습니다.
일반 회원의 경우 비밀번호 검증 후 탈퇴가 진행되도록 구현했으며,
소셜 로그인 회원은 별도의 비밀번호 입력 없이 탈퇴가 가능하도록 설계했습니다.
탈퇴가 완료되면 세션을 만료시켜 로그인 상태를 즉시 해제하도록 처리했습니다.

또한 로그인한 사용자 ID를 기준으로
사용자가 작성한 게시판 목록을 조회하여
본인이 작성한 게시글만 마이페이지에서 확인할 수 있도록 구현했습니다.

관심 지역 기능에서는
사용자가 등록한 관심 지역 목록을 조회할 수 있도록 했으며,
각 관심 지역에 대해 현재 미세먼지(PM10) 정보를 함께 조회해 제공하도록 구현했습니다.
불필요한 관심 지역은 REST 방식의 DELETE 요청을 통해
개별 삭제가 가능하도록 처리했습니다.

이를 통해 사용자가
회원 정보 관리, 활동 내역 확인, 관심 지역 관리까지
마이페이지에서 한 번에 관리할 수 있도록 구현했습니다.
