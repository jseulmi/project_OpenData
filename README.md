...
- 로그인한 사용자가 마이페이지에 접근하면 세션에 저장된 로그인 정보를 기준으로 회원 정보를 조회하여 최신 회원 정보를 화면에 표시하도록 구현했습니다.
...
- 수정 완료 후 회원 정보를 재조회하여 세션 정보를 즉시 갱신함으로써 변경 사항이 바로 반영되도록 처리합니다.
...
- 로그인한 사용자 ID를 기준으로 본인이 작성한 게시글 목록만 조회하도록 구현했습니다.
...
- 사용자가 등록한 관심 지역 목록 조회합니다.
- 각 관심 지역에 대해 현재 미세먼지(PM10) 정보를 함께 제공합니다.
- REST 방식의 DELETE 요청을 통해 관심 지역을 개별적으로 삭제할 수 있도록 구현했습니다.
